...
if ($id <= 0 || $name === '' || $category_id <= 0 || $price <= 0) {
    $_SESSION['flash_error'] = 'Valid food data is required.';
...
$stmt = $pdo->prepare("
    ...
    WHERE id = ?
");
$stmt->execute([$category_id, $name, $description, $price, $imageName, $available, $id]);

$_SESSION['flash_success'] = 'Food updated successfully.';
header('Location: index.php#foods');
exit;
?>